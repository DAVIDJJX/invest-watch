...
| 2026-09-18 | 停點 8 其餘（probe 彙整、互斥鎖、舊資料標示、schedule.json） | merge commit 用 `git log --merges -3` 找「停點 8-1～8-5」那一筆，`git revert -m 1 <它>`；或照 `docs/CHANGELOG.md` 用 `stop8-N` 標籤逐段退 |
...
- **盤中的輕量更新（平日每 30 分鐘）只抓「現價」需要的東西，不抓歷史檔。**
  ...
  本機完整更新實測 5 次（少掉當日匯率 CSV 與兩份 L6M 歷史檔），輕量更新少掉當日匯率 CSV 那一次。
